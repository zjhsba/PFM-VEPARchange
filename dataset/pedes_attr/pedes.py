...
class PedesAttr(data.Dataset):
    ...
    def __init__(self, cfg, split, transform=None, target_transform=None, idx=None):
        ...
        # ===== DUKE 分支 =====
        if cfg.DATASET.NAME == 'DUKE':
            dataset_info = pickle.load(open(cfg.DATASET.PKL, 'rb'))
            ...
            self.root_path = dataset_info.root if hasattr(dataset_info, 'root') and os.path.exists(dataset_info.root) else cfg.DATASET.ROOT
            ...
            if hasattr(dataset_info, 'attr_vectors'):
                self.label_vector = dataset_info.attr_vectors
            else:
                self.label_vector = np.zeros((len(self.attributes), 768))
    ...
    def __getitem__(self, index):
        ...
        if getattr(self, 'use_duke', False):
            ...
            img_dir = os.path.join(self.root_path, imgname)
            all_jpgs = sorted([f for f in os.listdir(img_dir) if f.lower().endswith(('.jpg', '.jpeg', '.png'))])

            # 取第2帧，不足则取第1帧
            rgb_idx = 1 if len(all_jpgs) > 1 else 0
            rgb_choice = all_jpgs[rgb_idx]

            rgb_img = Image.open(os.path.join(img_dir, rgb_choice)).convert('RGB')
            if self.transform is not None:
                rgb_img = self.transform(rgb_img)
            else:
                rgb_img = torch.from_numpy(np.array(rgb_img)).permute(2, 0, 1).float() / 255.0

            rgb_tensor = torch.stack([rgb_img])          # [1, C, H, W]
            event_tensor = torch.stack([rgb_img])        # [1, C, H, W] — 同帧复制
