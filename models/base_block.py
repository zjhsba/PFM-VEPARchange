...
from models.temporal_event_encoder import TemporalEventEncoder
...
class FeatClassifier(nn.Module):
    ...
    def __init__(self, backbone, classifier, bn_wd=True, enable_dct=True, dct_on_rgb=False, dct_on_event=True, 
        ...
        self.enable_temporal = (
            getattr(cfg, 'TEMPORAL', {}).get('ENABLE', False) if cfg is not None else False
        )
        if self.enable_temporal:
            t_cfg = cfg.TEMPORAL
            self.temporal_encoder = TemporalEventEncoder(
                dim=768,
                ...
                max_frames=t_cfg.get('MAX_FRAMES', 32),
                use_residual=t_cfg.get('USE_RESIDUAL', True),
            )

        # ----- Contrastive loss feature extraction -----
        self.enable_contrastive = (
            getattr(cfg, 'CONTRASTIVE', {}).get('ENABLE', False) if cfg is not None else False
        )
    ...
    def fresh_params(self):
        ...
        if self.enable_temporal:
            if self.bn_wd:
                params.extend(list(self.temporal_encoder.parameters()))
            else:
                params.extend(list(self.temporal_encoder.named_parameters()))
    ...
    def forward(self, image_rgb, image_event, label=None):
        ...
        if use_prompt_path:
            ...
            event_features = event_feat_map.view(batch_size, num_frames, N, D)
            if self.enable_temporal:
                event_features = self.temporal_encoder(event_features)  # [B, N, D]
            else:
                event_features = event_features.mean(dim=1)              # [B, N, D]

            # --- Capture pre-fusion features for contrastive loss (training only) ---
            # Placed BEFORE Hopfield / CrossModal so contrastive gradient stays clean.
            # .clone() creates an autograd fork: contrastive branch and classification branch
            # share the backbone but diverge before Hopfield.
            if self.training and self.enable_contrastive:
                rgb_contrast_feat = rgb_feat_map.clone()
                event_contrast_feat = event_features.clone()
            else:
                rgb_contrast_feat = None
                event_contrast_feat = None
        ...
        if self.training and self.enable_contrastive:
            return logits, feat, rgb_contrast_feat, event_contrast_feat
        else:
            return logits, feat
